...
use regex::Regex;
use opt_def::opt_def_parser;
use std::process::exit;
use std::panic::catch_unwind;

use clap::Parser;
use crate::arg_parser::{CmdLineElement, CmdLineTokenizer};
use crate::opt_def::{OptConfig, OptType, Target};
...
*/
fn shell_init_code(opt_cfg_list: &Vec<OptConfig>, cmd_line_args: &CmdLineArgs, typeset_vars: bool) -> Vec<CodeChunk> {
    ...
    for opt_cfg in opt_cfg_list {
        if let Target::Function(name) = &opt_cfg.target {
    ...
    for opt_cfg in opt_cfg_list {
        if typeset_vars {
            if let Target::Variable(name) = &opt_cfg.target {
                init_code.push(match &opt_cfg.opt_type {
                    OptType::Counter => CodeChunk::TypesetIntVar(name.clone()),
                    OptType::Assignment => CodeChunk::TypesetVar(name.clone()),
                    OptType::FlagAssignment(_) => CodeChunk::TypesetVar(name.clone()),
                    OptType::Flag => CodeChunk::TypesetVar(name.clone()),
                });
            }
        }
        if let OptType::Counter = &opt_cfg.opt_type {
            // only init variables, don't call functions
            match &opt_cfg.target {
...
fn assign_target(target: &Target, value:VarValue) -> CodeChunk {
    match target {
        Target::Variable(name) => CodeChunk::AssignVar(name.clone(), value),
        Target::Function(name) => CodeChunk::CallFunction(name.clone(), value),
    }
}

fn parse_shell_options(opt_cfg_list: &mut Vec<OptConfig>, cmd_line_args: &CmdLineArgs) -> Result<Vec<CodeChunk>, String> {
    ...
    for oss in &cmd_line_args.script_args {
        ...
        if result.is_ok() {
            ...
            return Err(format!("Invalid UTF-8 char(s) in {:?}", result.unwrap_err()));
        }
    }
    ...
    while let Some(e) = cl_tok.next() {
        if let CmdLineElement::Separator = e {
            ...
            if after_separator && cmd_line_args.remainder.is_some() {
                if let Some(array) = &cmd_line_args.remainder {
                    shell_code.push(CodeChunk::AddToArray(array.clone(), VarValue::StringValue(value)));
                }
            } else {
                if let Some(func) = &cmd_line_args.arg_callback {
                    shell_code.push(CodeChunk::CallFunction(func.clone(), VarValue::StringValue(value)));
                } else {
            ...
            let option =  (opt_cfg_list.iter_mut().find(|cfg| cfg.match_option(&e)), opt_value);
            ...
            if option.0.is_none() {
                ...
                if let OptType::Flag = oc.opt_type {
                    let bool_val = VarValue::BoolValue(some_str_to_bool(option.1, true));
                    shell_code.push(assign_target(&oc.target, bool_val));
                } else if let OptType::FlagAssignment(value) = &oc.opt_type {
                    // Conflict detection is done at end of processing.
                    let bool_val = some_str_to_bool(option.1, true);
                    if bool_val {
                        shell_code.push(assign_target(&oc.target, VarValue::StringValue(value.clone())));
                    }
                } else if let OptType::Assignment = oc.opt_type {
                    let opt_arg = match option.1 {
                        Some(v) => Some(v.clone()),
                        None => cl_tok.get_option_argument(),
                    };
                    if let Some(opt_arg) = opt_arg {
                        shell_code.push(assign_target(&oc.target, VarValue::StringValue(opt_arg)));
                    } else {
                        return Err(format!("Missing argument for: {}", e));
                    }
                } else if let OptType::Counter = oc.opt_type {
                    let value = match option.1 {
                        Some(v) => {
                            let cnt = match v.parse::<u16>() {
                                Ok(v) => v,
                                Err(_) => {
                                    return Err(format!("Invalid unsigned integer in value of option {}", e));
                                }
                            };
                            Some(cnt)
                        }
                    ...
                    */
                    shell_code.push(assign_target(&oc.target, VarValue::IntValue(oc.count_value as i32)));
                }
    ...
    for name in shell_name_table.keys() {
        if shell_name_table.get(name).unwrap().len() > 1  {
            ...
            if used_tab.len() > 1 {
                return Err(format!("Options are mutual exclusive: {}", used_tab.join(", ")));
            }
            if required && used_tab.len() == 0 {
                return Err(format!("One of the following options is required: {}", all_tab.join(", ")));
            }
        }

        for oc in &mut *opt_cfg_list {
            if oc.required && ! oc.assigned {
                return Err(format!("Required option not found: {}", oc.options_string()));
            }
        }
    }
...
fn parseargs(cmd_line_args: CmdLineArgs) -> !{
    ...
    let result = opt_def_parser::parse(&cmd_line_args.options);
    let mut opt_cfg_list = match result {
        Ok(list) => list,
        Err(error) => {
            die_internal(format!("Error parsing option definition: {}", error));
        },
    };
    ...
    if cmd_line_args.help_opt {
        opt_cfg_list.push(OptConfig {
            ...
            opt_type: OptType::Flag,
            target: Target::Function("show_help".to_string()),
            required: false,
            singleton: true,
            assigned: false,
            count_value: 0,
    ...
    if !shell_tmpl.supports_arrays && cmd_line_args.remainder.is_some() {
        die_internal(format!("Shell {} does not support arrays, so option -r/--remainder not supported", cmd_line_args.shell));
    }
